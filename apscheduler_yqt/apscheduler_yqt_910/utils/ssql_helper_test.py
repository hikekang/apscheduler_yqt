...
from utils.ssql_pool_helper import DataBase, config_QBBB,config_QBBA_net,config_QBBA
import traceback
...
def contain_keywords(keywords, str_match):
    # k_list=[]
    # for item in keywords.split("、"):
    #     k_list.append(item)
    # for item in k_list:
    #     if item in str_match:
    #         return True
    # return False
    '精测电子 '
    return any(k in str_match for k in keywords.split("、"))
...
def upload_many_data(data_list, industry_name, datacenter_id, info):
    ...
    if data_list:
        for work_id, data in enumerate(data_list):
            ...
            if db_qbba_net.execute(sql_qbb_a,tuple_data_qbb_a):
...
def mark_java_match_data(d,theam_list):
    ...
    for info in theam_list:
        if match_this_theam(info,match_data):
            ...
            item_dict.setdefault('create_date', datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))
            ...
            print(item_dict)
            t2 = datetime.datetime.strptime(item_dict['PublishDate_Std'], '%Y-%m-%d %H:%M')
            a_month_ago_date = (t2 - datetime.timedelta(days=7)).strftime('%Y-%m-%d %H:%M')
...
def get_month_data(time1, time2, industry_name,flushall=False):
    ...
    sql = "select industry_id,url from %s  with (NOLOCK) where publish_time between '%s' and '%s'" % (
...
def record_day_datas():
    ...
    for i in range(0,1):
        ...
        outfile = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"记录\\{record_time}", f"{record_time}.xlsx")
        print(date_now)
            ...
            for item in project_name:
                if data['customer'] == item:
                    ...
                    sql_qbbb = """ select count(*) from (select URL as url_count from 
                                    TS_DataMerge_Base with (NOLOCK) where C_Id='{0}' and PublishDate_Std between '{2}' 
                                    and '{1}' group by URL ) as temp
                                """.format(data['id'], date_now, date_yesterday)
                    print(sql_qbbb)
...
def record_error_info(error_info,customer=None):
    ...
    db_qbbb.execute(sql,(customer,create_date,error_info))
