...
def find_curent_num(start_time, end_time, myconfig, info, yqt_count):
    ...
    # count_A = db_my_qbba.execute_query(sql_A)[0][0]
    count_A = db_my_qbba_net.execute_query(sql_A)[0][0]
    # TODO 后续替换
    # count_A_net = db_my_qbba_net.execute_query(sql_A)[0][0]
    ...
    count_B = db_my_qbbb.execute_query(sql_B)[0][0]
...
def find_day_data_count(myconfig):
    ...
    for item in p_data:
        ...
        count_B += int(db_my_qbbb.execute_query(sql_B)[0][0])

    sql_insert_a = "insert into record_log_industry (industry,record_time,TS_A_num,QBB_B_num) values (%s,%s,%d,%d)"

    # db_my_qbba.execute(sql_insert_a, (industry_name, end_time, count_A, count_B))
    # db_my_qbba_net.execute(sql_insert_a, (industry_name, end_time, count_A, count_B))

    db_my_qbba_net.execute(sql_insert_a, (industry_name, end_time, count_A_net, count_B))
    return count_B
...
def get_industry_keywords():
    ...
    # get enable project
    data = db_my_qbbb.execute_query(sql_qbbb)
    new_data = []
    for d in iter(data):
        # 处理乱码
        new_d = {
        ...
        for i, dd in enumerate(d):
            if i == 0:
                new_d['id'] = dd
                # 单字段去重
                sql_QBBA = "select Word,SimultaneousWord,Excludeword from TS_Keywords where C_ID={} " \
                           "group by Word,SimultaneousWord,Excludeword".format(dd)

                # data_A = db_my_qbba.execute_query(sql_QBBA)
                # TODO 后续替换
                data_A_net = db_my_qbba_net.execute_query(sql_QBBA)
                for da in data_A_net:
                    ...
                    for i, d_a in enumerate(da):
                        ...
                        if d_a and i == 1:
                            ...
                            new_d['yqt_keywords'] += "+(" + re.sub('、', '|', d_a.encode('latin1').decode('gbk')) + "))|"
                            project_word['keywords'] += "+(" + re.sub('、', '|',
                                                                      d_a.encode('latin1').decode('gbk')) + "))"
...
def post_data(data_list, industry_name):
    ...
    for data in data_list:
        ...
        # print("执行")
        # 插入QBB_A库
        # db_my_qbba.execute(sql_qbb_a)

        db_my_qbba_net.execute(sql_qbb_a)
...
def upload_many_data_java(data_list, industry_name, datacenter_id):
    ...
    for d in tuple_data_list_qbb_a:
        # db_my_qbba.execute(sql_qbb_a, d)

        db_my_qbba_net.execute(sql_qbb_a, d)
...
def upload_many_data(data_list, industry_name, datacenter_id, info):
    ...
    if data_list:
        for work_id, data in enumerate(data_list):
            ...
            try:
                ...
                error_info="%s:Java程序--%s"%(info['customer'],traceback.format_exc())
    ...
    """
    sql_of_Tid = "select T_Id,Word,SimultaneousWord,ExcludeWord from TS_Keywords where C_Id=%s group by" \
                 " T_Id,Word,SimultaneousWord,ExcludeWord" % info['id']

    thream_info_list = []

    # First check the topic ID based on the customer ID
    # T_id = db_my_qbba.execute_query(sql_of_Tid)
    # TODO 后续替换
    T_id_net = db_my_qbba_net.execute_query(sql_of_Tid)
    ...
    def load_data(d):
        ...
        if flag == 0:
            ...
            URL_DATA = db_my_qbbb.execute_query(SQL_SELECT)[0]
            # 更新redis
...
def match_insert_alone_data(data, info, T_id):
    ...
    :return:
    """
    curent_date_time = datetime.datetime.strptime(data['时间'], "%Y-%m-%d %H:%M:%S")
    year = curent_date_time.year
    month = curent_date_time.month
    day = curent_date_time.day
...
def mark_java_match_data(d, theam_list):
    ...
    # 词云
    word_post_data = {
        'queues': 'qbb.task.msg.wordcloud_2.1',
    ...
    for info in theam_list:
        if match_this_theam(info, match_data):
            ...
            # 情感处理发送的数据
            emotion_2_data = {
                'queues': 'qbb.task.msg.emotion_2.1',
                'message': str(post_data)
            }
            url = 'http://localhost:8090/jms/send'
            # 单条
            ...
            tag_data = {
                'queues': 'qbb.task.msg.tags_2.1',
                'message': str({"sN": d['SN'], "cId": d['C_Id']})
            }
            requests.get(url=url, proxies=proxies, params=tag_data)

            curent_date_time=datetime.datetime.strptime(item_dict['PublishDate_Std'],"%Y-%m-%d %H:%M:%S")
            year=curent_date_time.year
            month=curent_date_time.month
            day=curent_date_time.day
            y_m=str(year)+"_"+str(month)
            ...
            # 数据插入B库
            sql_of_base = 'insert into TS_DataMerge_Base_{} ({}) values ({})'. \
                format(y_m,",".join(iisql.keys()), ",".join(['%s'] * len(iisql.keys())))
            ...
            similar_news_sql = f"select SN,PublishDate_Std from TS_DataMerge_Base_{y_m} with (NOLOCK) where Title like '%{item_dict['Title']}%' " \
                               f"and PublishDate_Std between   '{a_month_ago_date}' and '{item_dict['PublishDate_Std']}' order by PublishDate_Std,SN asc"
            print(similar_news_sql)
            similar_result_pre = db_my_qbbb.execute_query(similar_news_sql_pre)
            similar_result = db_my_qbbb.execute_query(similar_news_sql)
            similar_result=similar_result+similar_result_pre
...
def sub_class_match_data(info, d):
    ...
    # 获得当前监测主题下的所有子分类
    sub_class_datas = db_my_qbbb.execute_query(sql)
    ...
    def match_sub_class_data(class_info, data):
        ...
        # 查看当前分类下面是否有子分类
        item_sub_class = db_my_qbbb.execute_query(sql_sub_class)
...
def get_month_data(time1, time2, industry_name, flushall=False):
    ...
    """
    if flushall:
        myredis.redis.delete(tables[industry_name])
    sql = "select industry_id,url from %s where publish_time between '%s' and '%s'" % (
        tables[industry_name], time1, time2)
    # datas = db_my_qbba.execute_query(sql)
    # TODO 后续替换
    datas_net = db_my_qbba_net.execute_query(sql)
...
def record_log(data):
    ...
    # db_my_qbba.execute(sql_record, data)

    db_my_qbba_net.execute(sql_record, data)
    # my_e = my_Email()
...
def customer_log():
    ...
    for customer in get_industry_keywords():
        ...
        # qbbb 库查询的数量
        today_customer_num = db_my_qbbb.execute_query(sql_qbbb)[0][0]
        # 记录项目的数据量
        ...
        for data in get_industry_keywords():
            for item in project_name:
                if data['customer'] == item:
                    print(data['customer'])
                    sql_qbbb = "select count(*) from dbo.TS_DataMerge_Base where C_Id='{0}' and PublishDate_Std " \
                               "between '{2}' and '{1}'".format(data['id'], date_now, date_yesterday)
                    # print(sql_qbbb)
                    sql_num_B = db_my_qbbb.execute_query(sql_qbbb)[0][0]
                    sql_qbbb_source_type = "select source_type,count(*) from dbo.TS_DataMerge_Base  where C_Id='{0}'  and " \
                                           "PublishDate_Std between '{2}' and '{1}' group by source_type".format(
                        data['id'], date_now, date_yesterday)
                    source_type = db_my_qbbb.execute_query(sql_qbbb_source_type)
                    count_source_type = {
                        '网媒': 0,
                        '问答': 0,
                        '贴吧': 0,
                    ...
                    try:
                        ...
                        print(sq_tsa)
                        if sq_tsa:
                            # sql_a = db_my_qbba.execute_query(sq_tsa)[0][0]
                            # TODO 后续替换
                            sql_a_net = db_my_qbba_net.execute_query(sq_tsa)[0][0]
                        else:
                            sql_a = 0
                        # sql_a=0
                        source_type_list = list(count_source_type.values())
                        print(source_type_list)
                        spide_helper.all_project_save_record_day(outfile, sql_a, sql_num_B, data['customer'],
                                                                 data['industry_name'], source_type_list)
                        spide_helper.all_project_save_record_day(outfile, sql_a_net, sql_num_B, data['customer'],
                                                                 data['industry_name'], source_type_list)
                    except Exception as e:
                        print(e)

def record_error_info(error_info,customer=None):
    create_date=(datetime.datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
    sql="insert into dbo.TS_Spider_Error (customer,create_date,error_info) values (%s,%s,%s)"
    db_my_qbbb.execute(sql,(customer,create_date,error_info))
